...
В панели есть кнопка `Очистить историю`.

Что она делает:

- не удаляет логи безвозвратно;
- переносит старые log-файлы в архив внутри исходного проекта;
- не трогает файлы за текущий день;
- не трогает файлы, которые изменялись в последние 10 минут;
- после переноса автоматически пересобирает индекс панели.

Архив складывается в каталог вида:

```text
.query-analytics-archive/<timestamp>/
```
